<script lang="ts">
    ...
    $: currentRating = (id && $ratingOverrides[id] !== undefined)
        ? $ratingOverrides[id]
        : (rating ?? 0);
    ...
    $: if (id && itemType && rating === undefined && $ratingOverrides[id] === undefined) {
...
<div class="flex items-center">
    {#each Array(5) as _, i}
        ...
        <span
            ...
            class="px-0.5 py-0.5 {interactive ? 'cursor-pointer' : 'cursor-default'}"
            on:mouseenter={() => { if (interactive) hovered = i; }}
            on:mouseleave={() => { if (interactive) hovered = -1; }}
            on:click|preventDefault|stopPropagation={() => handleRate(i)}
        >
            {#if i < displayRating}
                <IconStarFilled {size} style="color: {rated_color}" />
            {:else}
                <IconStarFilled {size} style="color: {unrated_color}; opacity: 0.75" />
            {/if}
        </span>
    {/each}
</div>
